# modify from https://github.com/opendilab/DI-engine/blob/main/ding/example/dqn.py
...
# # from configs.stocks_dqn_config import main_config, create_config
...
def main():
    ...
    for i in range(4):
        ...
        cfg2 = compile_config(main_config, create_cfg=create_config, 
                                auto=True, save_cfg=task.router.node_id == 0)
        ...
        cfg.policy.learn.learner.dataloader.num_workers = 64
        cfg.exp_name = EXP_NAME
        # cfg.policy.learn.learner.train_iterations = 100
        
        
        print(cfg)
        print(cfg.env.manager)
        ...
        with task.start(async_mode=False, ctx=OnlineRLContext()):
            collector_env = BaseEnvManagerV2(
                ...
                env_fn=[lambda: StocksEnv(cfg.env) for _ in range(cfg.env.collector_env_num)],
                cfg=cfg.env.manager
            )

            evaluator_env = BaseEnvManagerV2(
                # env_fn=[lambda: DingEnvWrapper(gym.make("CartPole-v0")) for _ in range(cfg.env.evaluator_env_num)],
                env_fn=[lambda: StocksEnv(cfg.env) for _ in range(cfg.env.evaluator_env_num)],
                cfg=cfg.env.manager
            )
